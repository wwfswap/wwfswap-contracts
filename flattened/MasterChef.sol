...
// File: contracts\libs\IWWFReferral.sol



pragma solidity 0.6.12;

interface IWWFReferral {
    /**
     * @dev Record referral.
     */
...
// WWFToken with Governance.
contract WWFToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 WWFs)
    ...
    IUniswapV2Router02 public wwfRouter;
    // The trading pair
    address public wwfPair;
    // In swap and liquify
    ...
    event WWFRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "WWF::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the WWFToken contract.
     */
    constructor() public BEP20("WWF Swap", "WWF") {
    ...
    /// @dev overrides transfer function to meet tokenomics of WWF
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(wwfRouter) != address(0)
            && wwfPair != address(0)
            && sender != wwfPair
            && sender != owner()
        ) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "WWF::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "WWF::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = wwfRouter.WETH();

        _approve(address(this), address(wwfRouter), tokenAmount);

        // make the swap
        wwfRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        wwfRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from wwfRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "WWF::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "WWF::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "WWF::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateWWFRouter(address _router) public onlyOperator {
        wwfRouter = IUniswapV2Router02(_router);
        wwfPair = IUniswapV2Factory(wwfRouter.factory()).getPair(address(this), wwfRouter.WETH());
        require(wwfPair != address(0), "WWF::updateWWFRouter: Invalid pair address.");
        emit WWFRouterUpdated(msg.sender, address(wwfRouter), wwfPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "WWF::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "WWF::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "WWF::delegateBySig: invalid nonce");
        require(now <= expiry, "WWF::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "WWF::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying WWFs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "WWF::_writeCheckpoint: block number exceeds 32 bits");
